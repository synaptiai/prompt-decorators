...
class TestDecoratorName:
    ...
    def test_initialization_default_params(self, load_decorator):
        ...
        assert decorator.name == "DecoratorName"

    def test_parameterName_type_validation(self, load_decorator):
        """Test parameterName type validation."""
        decorator_class = load_decorator("DecoratorName")
        assert decorator_class is not None
    ...
    def test_parameterName_min_validation(self, load_decorator):
        """Test parameterName minimum value validation."""
        decorator_class = load_decorator("DecoratorName")
        assert decorator_class is not None
        params = self._get_valid_params()
        with pytest.raises(ValidationError) as exc_info:
    ...
    def test_parameterName_max_validation(self, load_decorator):
        """Test parameterName maximum value validation."""
        decorator_class = load_decorator("DecoratorName")
        assert decorator_class is not None
        params = self._get_valid_params()
        with pytest.raises(ValidationError) as exc_info:
    ...
    def test_metadata(self, load_decorator):
        ...
        assert metadata["name"] == "DecoratorName"
