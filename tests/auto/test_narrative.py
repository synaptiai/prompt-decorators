...
from prompt_decorators.decorators import Narrative
...
class TestNarrative:
    ...
    def test_initialization_default_params(self, load_decorator):
        ...
        decorator_class = load_decorator("Narrative")
        assert decorator_class is not None
        decorator = decorator_class()
        assert decorator is not None
        assert decorator.name == "Narrative"
    ...
    def test_characters_type_validation(self, load_decorator):
        """Test characters type validation."""
        decorator_class = load_decorator("Narrative")
        assert decorator_class is not None
        params = self._get_valid_params()
        params['characters'] = 'invalid'
        with pytest.raises(ValidationError) as exc_info:
            decorator_class(**params)
    ...
    def test_metadata(self, load_decorator):
        ...
        assert metadata["name"] == "Narrative"
