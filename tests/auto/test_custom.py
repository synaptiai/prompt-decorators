...
from prompt_decorators.decorators import Custom
...
class TestCustom:
    ...
    def test_initialization_default_params(self, load_decorator):
        ...
        decorator_class = load_decorator("Custom")
    ...
    def test_rules_required(self, load_decorator):
        """Test that rules is required."""
        decorator_class = load_decorator("Custom")
        assert decorator_class is not None
        params = self._get_valid_params()
        del params["rules"]
    ...
    def test_rules_type_validation(self, load_decorator):
        """Test rules type validation."""
        decorator_class = load_decorator("Custom")
        assert decorator_class is not None
        params = self._get_valid_params()
        params['rules'] = 123
    ...
    def test_name_type_validation(self, load_decorator):
    ...
    def test_priority_type_validation(self, load_decorator):
        """Test priority type validation."""
        decorator_class = load_decorator("Custom")
        assert decorator_class is not None
        params = self._get_valid_params()
        params['priority'] = 'invalid_enum_value'
        with pytest.raises(ValidationError) as exc_info:
            decorator_class(**params)
        assert "priority" in str(exc_info.value)
        assert "one of" in str(exc_info.value).lower()

    def test_priority_enum_validation(self, load_decorator):
        """Test priority enum value validation."""
        decorator_class = load_decorator("Custom")
        assert decorator_class is not None
        params = self._get_valid_params()
        params['priority'] = 'invalid_enum_value'
