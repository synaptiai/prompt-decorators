...
from prompt_decorators.decorators import Motivational
...
class TestMotivational:
    ...
    def test_initialization_default_params(self, load_decorator):
        ...
        decorator_class = load_decorator("Motivational")
        assert decorator_class is not None
        decorator = decorator_class()
        assert decorator is not None
        assert decorator.name == "Motivational"
    ...
    def test_actionable_type_validation(self, load_decorator):
        """Test actionable type validation."""
        decorator_class = load_decorator("Motivational")
        assert decorator_class is not None
        params = self._get_valid_params()
        params['actionable'] = 'invalid'
        with pytest.raises(ValidationError) as exc_info:
            decorator_class(**params)
        assert "actionable" in str(exc_info.value)
    ...
    def test_metadata(self, load_decorator):
        ...
        assert metadata["name"] == "Motivational"
