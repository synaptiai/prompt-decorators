...
from prompt_decorators.decorators import Limitations
...
class TestLimitations:
    ...
    def test_initialization_default_params(self, load_decorator):
        ...
        decorator_class = load_decorator("Limitations")
        assert decorator_class is not None
        decorator = decorator_class()
        assert decorator is not None
        assert decorator.name == "Limitations"

    def test_detail_type_validation(self, load_decorator):
        """Test detail type validation."""
        decorator_class = load_decorator("Limitations")
        assert decorator_class is not None
        params = self._get_valid_params()
        params['detail'] = 'invalid_enum_value'
        with pytest.raises(ValidationError) as exc_info:
            decorator_class(**params)
        assert "detail" in str(exc_info.value)
        assert "one of" in str(exc_info.value).lower()

    def test_detail_enum_validation(self, load_decorator):
        """Test detail enum value validation."""
        decorator_class = load_decorator("Limitations")
        assert decorator_class is not None
        params = self._get_valid_params()
        params['detail'] = 'invalid_enum_value'
    ...
    def test_metadata(self, load_decorator):
        ...
        assert metadata["name"] == "Limitations"
