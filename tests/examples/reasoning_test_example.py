...
def test_reasoning_basic_reasoning_process():
    ...
    check_expectation(response, "matches_description", description="Provides a logical explanation of why the sky appears blue, showing the reasoning process that leads to the conclusion")


def test_reasoning_comprehensive_scientific_explanation():
    """Test Reasoning with example: Comprehensive scientific explanation with detailed reasoning."""
    response = llm_client.generate("+++Reasoning(depth=comprehensive)\nDescribe how neural networks learn patterns in data.")
    
    # Check if response meets expectations
    check_expectation(response, "matches_description", description="Delivers a comprehensive explanation of neural network learning with detailed reasoning steps, including gradient descent, backpropagation, and the mathematics behind pattern recognition")
...
def test_reasoning_compatibility_with_step_by_step():
    ...
    assert "reason" in response.lower() or "because" in response.lower() or "therefore" in response.lower()
