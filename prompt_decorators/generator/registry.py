...
class RegistryScanner:
    ...
    def __init__(self, registry_path: Union[str, Path]):
        ...
        if not self.registry_path.exists() or not self.registry_path.is_dir():
            raise ValueError(f"Registry path '{registry_path}' is not a valid directory")
        ...
        if "parameters" in data:
            for i, param in enumerate(data["parameters"]):
                ...
                if "type" not in param:
                    logger.warning(f"Parameter '{param.get('name', f'at index {i}')}' missing 'type' in {file_path}")
                    return False
...
if __name__ == "__main__":
    ...
    try:
        ...
        for category, category_decorators in scanner.decorators_by_category.items():
            ...
            print(f"  {category} ({len(category_decorators)}): {', '.join(decorator_names)}")
