    ...
    def __init__(
        ...
        if self._citationStyle is not None:
            if not isinstance(self._citationStyle, str):
                raise ValidationError(
                    "The parameter 'citationStyle' must be a string type value."
                )
            if self._citationStyle not in ["APA", "MLA", "Chicago", "Harvard", "IEEE"]:
                raise ValidationError(
                    f"The parameter 'citationStyle' must be one of the allowed enum values: ['APA', 'MLA', 'Chicago', 'Harvard', 'IEEE']. Got {self._citationStyle}"
                )
