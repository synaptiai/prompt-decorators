...
import logging
import threading
from typing import Dict, Any, Optional, List, Type, Tuple, cast
...
class DecoratorCache:
    ...
    def _evict_definitions(self, count: int) -> int:
        ...
        sorted_keys = sorted(self._definitions.keys(), 
                            key=lambda k: self._definitions[k][0])
    ...
    def _evict_instances(self, count: int) -> int:
        ...
        """
        # Sort by timestamp (oldest first)
        sorted_keys = sorted(self._instances.keys(), 
                            key=lambda k: self._instances[k][0])
