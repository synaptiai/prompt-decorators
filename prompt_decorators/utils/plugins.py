...
from typing import Dict, List, Any, Optional, Type, Callable, Set, Union, Tuple, cast

from prompt_decorators.core.base import BaseDecorator
from prompt_decorators.utils.discovery import DecoratorRegistry, get_registry
...
class PluginManager:
    ...
    def load_plugin(self, plugin: Plugin) -> bool:
        ...
        if plugin.name in self._plugins:
            logger.warning(f"Plugin {plugin.name} already loaded. Unload first to replace.")
            return False
            ...
            try:
                ...
                if hasattr(module, "plugin_info"):
                    ...
                    if "name" not in plugin_info or "version" not in plugin_info:
                        logger.warning(f"Missing required fields in plugin_info for {package_name}")
                        return None
                    ...
                    for _, submodule_name, is_pkg in pkgutil.iter_modules([str(package_path)]):
                        try:
                            ...
                            for name, obj in inspect.getmembers(submodule):
                                if (inspect.isclass(obj) and 
                                    obj.__module__ == submodule.__name__ and
                                    issubclass(obj, BaseDecorator) and 
                                    obj != BaseDecorator):
                                    decorators.append(obj)
        ...
        try:
            ...
            try:
                ...
                if hasattr(module, "plugin_info"):
                    ...
                    if "name" not in plugin_info or "version" not in plugin_info:
                        logger.warning(f"Missing required fields in plugin_info for {module_name}")
                        return None
    ...
    def _load_plugin_from_json(self, json_path: Path) -> Optional[Plugin]:
        ...
        try:
            ...
            if "name" not in plugin_info or "version" not in plugin_info or "decorators" not in plugin_info:
    ...
    def start_watching_directories(self, interval: int = 10) -> None:
        ...
        self._watch_thread = threading.Thread(
            target=self._watch_directories_thread,
            daemon=True
        )
            ...
            try:
                ...
                for directory in self._plugin_directories:
                    ...
                    for item in directory_path.glob("**/*"):
                        ...
                        if item_path not in last_modified_times or current_mtime > last_modified_times[item_path]:
                            logger.info(f"Detected change in {item_path}")
                            
                            # Reload plugin
                            if item.suffix == ".py" or item.suffix == ".json" or item.name == "__init__.py":
                                ...
                                for update_item in directory_path.glob("**/*"):
                                    if update_item.is_file():
                                        last_modified_times[str(update_item)] = update_item.stat().st_mtime
