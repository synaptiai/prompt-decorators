...
class CompatibilityIssue:
    ...
    def __init__(
        ...
        self.decorator1 = decorator1.name if isinstance(decorator1, BaseDecorator) else decorator1
        self.decorator2 = decorator2.name if isinstance(decorator2, BaseDecorator) else decorator2
...
class CompatibilityChecker:
    ...
    def check_compatibility(
        ...
        for rule in self._compatibility_rules.get(pair, []):
            if not rule.get("compatible", True):
                issues.append(
                    CompatibilityIssue(
                        message=rule.get("message", f"{name1} is incompatible with {name2}"),
                        decorator1=decorator1,
                        decorator2=decorator2,
                        severity=rule.get("severity", CompatibilityIssue.SEVERITY_ERROR)
                    )
                )
    ...
    def check_compatibility_group(
        self, 
        decorators: List[Union[str, BaseDecorator]]
    ) -> List[CompatibilityIssue]:
        """
...
def setup_core_compatibility_rules() -> None:
    ...
    checker.add_rule("Reasoning", "OutputFormat", {
        "compatible": True,
    ...
    checker.add_rule("StepByStep", "DetailLevel", {
        "compatible": True,
    ...
    checker.add_rule("OutputFormat", "CodeGeneration", {
        "compatible": True,
