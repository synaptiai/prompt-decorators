...
from typing import Dict, List, Any, Optional, Set, Union, Callable, Tuple, Type, get_type_hints
...
class DocGenerator:
    ...
    def _extract_function_doc(self, func) -> Dict[str, Any]:
        ...
        return_type = None if return_annotation is signature.empty else str(return_annotation)
    ...
    def _extract_param_types_from_docstring(self, docstring: str) -> Dict[str, str]:
        ...
        # Look for Google-style docstring
        pattern = r"Args:\s*(?:\n\s*([^:]+): [^(]+ \(([^)]+)\)|\n\s*([^:]+): )"
        matches = re.findall(pattern, docstring)
        
        for match in matches:
            if match[0]:  # Parameter with type
                param_name = match[0].strip()
                param_type = match[1].strip()
                param_types[param_name] = param_type
            elif match[2]:  # Parameter without type
                param_name = match[2].strip()
    ...
    def merge_code_and_registry_docs(self) -> Dict[str, Dict[str, Any]]:
        """
        Merge code documentation with registry metadata.
        
        Returns:
            Dictionary with merged documentation
        """
        merged_docs = {
        ...
        for decorator_name, registry_def in self.registry_data.items():
            ...
            for class_path, class_doc in self.classes_doc.items():
    ...
    def generate_markdown_docs(self, output_dir: Optional[str] = None) -> None:
        ...
        for module_path, module_doc in merged_docs["modules"].items():
            self._generate_module_markdown(module_path, module_doc, merged_docs, output_dir)
    ...
    def _generate_index_markdown(self, docs: Dict[str, Any], output_dir: str) -> None:
        ...
        for module_path, module_doc in docs["modules"].items():
        ...
        index_content += "\n## Decorators\n\n"
        for decorator_name, decorator_doc in docs["decorators"].items():
    ...
    def _generate_module_markdown(
        ...
        Args:
            ...
            docs: The full documentation data
            output_dir: Directory where documentation should be written
        """
        # Skip internal modules
        if "._" in module_path:
    ...
    def _generate_decorator_markdown(
        ...
        Args:
            decorator_name: The decorator name
            decorator_doc: The decorator documentation
            output_dir: Directory where documentation should be written
        """
        ...
        if "parameters" in registry_def:
            ...
            for param in registry_def["parameters"]:
                ...
                decorator_content += f"**Required:** {'Yes' if param_required else 'No'}  \n"
            ...
            if methods:
                ...
                for method_name, method_doc in methods.items():
                    ...
                    if return_type:
                        ...
                        decorator_content += f"**Signature:** `{method_name}({param_str})`\n\n"
    ...
    def _format_function_markdown(self, func_name: str, func_doc: Dict[str, Any]) -> str:
        """
