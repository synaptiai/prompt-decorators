            ...
            if issubclass(self.expected_type, Enum) and isinstance(value, str):
                try:
                    ...
                    if isinstance(e, ValidationError):
                        raise
                    raise ValidationError(
                        decorator_name, param_name, f"Cannot convert '{value}' to {self.expected_type.__name__}"
                    )
...
class RangeValidator(Validator):
    ...
    def validate(
        ...
        if not isinstance(value, (int, float)):
            raise ValidationError(
                decorator_name, param_name, f"Expected number, got {type(value).__name__}"
            )
        
        # Check range
        if self.minimum is not None and value < self.minimum:
            raise ValidationError(
                decorator_name, param_name, f"Value {value} is below minimum {self.minimum}"
            )
        if self.maximum is not None and value > self.maximum:
            raise ValidationError(
                decorator_name, param_name, f"Value {value} is above maximum {self.maximum}"
            )
...
class PatternValidator(Validator):
    ...
    def validate(self, decorator_name: str, param_name: str, value: Any) -> Optional[str]:
        """
        ...
        if not isinstance(value, str):
            raise ValidationError(
                decorator_name, param_name, f"Expected string, got {type(value).__name__}"
            )
        
        # Check pattern
        if not self.pattern.match(value):
            raise ValidationError(
                decorator_name, param_name, f"Value '{value}' does not match pattern '{self.pattern.pattern}'"
            )
...
class EnumValidator(Validator):
    ...
    def validate(self, decorator_name: str, param_name: str, value: Any) -> Optional[Enum]:
        """
...
class ListValidator(Validator):
    ...
    def validate(self, decorator_name: str, param_name: str, value: Any) -> Optional[List[Any]]:
        """
        ...
        if self.min_length is not None and len(value) < self.min_length:
            raise ValidationError(
                decorator_name, param_name, f"List length {len(value)} is below minimum {self.min_length}"
            )
        if self.max_length is not None and len(value) > self.max_length:
            raise ValidationError(
                decorator_name, param_name, f"List length {len(value)} is above maximum {self.max_length}"
            )
...
class DictValidator(Validator):
    ...
    def validate(self, decorator_name: str, param_name: str, value: Any) -> Optional[Dict[Any, Any]]:
        """
        ...
        if not isinstance(value, dict):
            raise ValidationError(
                decorator_name, param_name, f"Expected dictionary, got {type(value).__name__}"
            )
...
class ValidationPipeline:
    ...
    def validate(self, decorator_name: str, parameters: Dict[str, Any]) -> Dict[str, Any]:
        """
